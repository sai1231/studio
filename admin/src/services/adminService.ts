...
export async function getUsersWithDetails(): Promise<AdminUser[]> {
    ...
    return [...mockUsers, ...allUsersFromDb].sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
}
...
export function subscribeToUserCount(callback: (count: number) => void): Unsubscribe {
    const usersCollectionRef = collection(db, 'users');
...
export function subscribeToContentCount(callback: (count: number) => void): Unsubscribe {
    const contentCollectionRef = collection(db, 'content');
    return onSnapshot(contentCollectionRef, (snapshot) => {
        callback(snapshot.size);
    }, (error) => {
        console.error("Error subscribing to content count:", error);
        callback(0);
    });
}

export function subscribeToNewUserCount(callback: (count: number) => void): Unsubscribe {
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
    const q = query(collection(db, 'users'), where('createdAt', '>=', Timestamp.fromDate(sevenDaysAgo)));
    
    return onSnapshot(q, (snapshot) => {
        callback(snapshot.size);
    }, (error) => {
        console.error("Error subscribing to new user count:", error);
        callback(0);
    });
}

export async function subscribeToProUserCount(callback: (count: number) => void): Promise<Unsubscribe> {
    try {
        const rolesQuery = query(collection(db, 'roles'), where('name', 'in', ['pro_user', 'admin']));
        const rolesSnapshot = await getDocs(rolesQuery);

        if (rolesSnapshot.empty) {
            console.warn("No 'pro_user' or 'admin' role found for subscription count.");
            callback(0);
            return () => {}; // No-op unsubscribe
        }

        const proRoleIds = rolesSnapshot.docs.map(doc => doc.id);
        
        const usersQuery = query(collection(db, 'users'), where('roleId', 'in', proRoleIds));

        return onSnapshot(usersQuery, (snapshot) => {
            callback(snapshot.size);
        }, (error) => {
            console.error("Error subscribing to pro user count:", error);
            callback(0);
        });
    } catch (e) {
        console.error("Error setting up pro user subscription:", e);
        callback(0);
        return () => {};
    }
}

export function subscribeToRecentUsers(callback: (users: DashboardRecentUser[]) => void): Unsubscribe {
    const q = query(collection(db, 'users'), orderBy('createdAt', 'desc'), limit(5));
    return onSnapshot(q, (snapshot) => {
        const recentUsers: DashboardRecentUser[] = snapshot.docs.map(doc => {
            const data = doc.data();
            return {
                id: doc.id,
                email: data.email || '',
