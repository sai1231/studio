    ...
    <Card className={cn(
      ...
      <Link href={`/content/${item.id}`} passHref className="flex flex-col flex-grow group/link">
        <div className="flex flex-col flex-grow cursor-pointer">
          ...
          <CardHeader className={cn(!hasImage && "pt-4 pb-3")}>
            {!hasImage && (
          ...
          <CardContent className={cn("flex-grow space-y-3", hasImage ? "pt-0" : "pt-2")}>
            {item.description && (
            ...
            <div className="flex flex-wrap gap-1.5">
              {item.domain && (
                <Badge variant="outline" className="text-xs py-0.5 px-1.5 border-blue-500/30 text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30">
                  <Landmark className="h-3 w-3 mr-1 opacity-70" />{item.domain}
                </Badge>
              )}
              {item.contentType && (
                <Badge variant="outline" className="text-xs py-0.5 px-1.5 border-purple-500/30 text-purple-700 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/30">
            ...
            {item.tags && item.tags.length > 0 && (
              <div className="flex flex-wrap gap-2 pt-1">
                {item.tags.slice(0, 3).map((tag) => (
                  <Badge key={tag.id} variant="secondary" className="font-normal bg-opacity-70 dark:bg-opacity-50">
                    {tag.name}
                  </Badge>
                ))}
      ...
      <CardFooter className={cn("flex justify-between items-center pt-3 border-t")}>
        ...
        <div className="flex items-center gap-2">
          {item.type === 'link' && item.url && (
            <Button variant="outline" size="icon" asChild onClick={handleActionClick} className={cn(specifics.bgClass !== 'bg-card' && 'bg-background/30 hover:bg-background/50 border-foreground/20')}>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" onClick={handleActionClick} className={cn(specifics.bgClass !== 'bg-card' && 'hover:bg-black/10 dark:hover:bg-white/10')}>
