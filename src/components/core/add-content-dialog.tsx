...
const AddContentDialog: React.FC<AddContentDialogProps> = ({ open, onOpenChange, zones, onContentAdd, children }) => { 
  ...
  useEffect(() => {
    if (open) {
      ...
      // Ensure placeholder is set for general use
      editor?.extensionManager.extensions.filter(ext => ext.name === 'placeholder').forEach(ext => {
        ext.options.placeholder = 'Write your content or note here...';
      });
  ...
  const dialogDescriptionText = "Provide a URL to save a link, or just add a title and content to save a note. Select a zone.";
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex-grow overflow-y-auto pr-2 space-y-4 py-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="description" className="font-medium">
              {`Content ${!!watchedUrl ? "(Optional for links)" : "(Required for notes)"}`}
            </Label>
