...
const AddTodoDialog: React.FC<AddTodoDialogProps> = ({ open, onOpenChange }) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg md:max-w-xl max-h-[80vh] flex flex-col bg-card p-0">
        ...
        <div className="flex-grow flex flex-col min-h-0">
            <div className="px-6 pb-4 border-b">
                <form onSubmit={handleSubmit} className="flex items-start gap-2 flex-shrink-0">
                <Input
                    id="todo-title"
                    ref={titleInputRef}
                    value={todoTitle}
            ...
            <ScrollArea className="flex-grow mt-2 pr-1 custom-scrollbar px-6">
            {isLoadingTodos ? (
                <div className="flex justify-center items-center py-10">
                <Loader2 className="h-8 w-8 animate-spin text-primary" />
                </div>
            ) : tasks.length === 0 ? (
                <p className="text-center text-muted-foreground py-10">No TODOs yet. Add your first one!</p>
            ) : (
