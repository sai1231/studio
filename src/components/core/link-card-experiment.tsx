...
// Copied from original link-card.tsx
...
const ContentCardExperiment: React.FC<ContentCardProps> = ({ item, onEdit, onDelete }) => {
  ...
  const imageAiHint = item.imageUrl ? item.title.split(' ').slice(0,2).join(' ') : "abstract design";


  // Requirement 1: For image type, just show image
  if (item.type === 'image' && item.imageUrl) {
    return (
      <Card
        className={cn(
          "overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 group rounded-2xl break-inside-avoid mb-4 cursor-pointer",
          "aspect-square" // Making image cards square for a start
        )}
        onClick={() => onEdit(item)} // Still allow edit on click of the image
      >
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      className={cn("h-9 w-9 bg-destructive/80 hover:bg-destructive text-destructive-foreground shadow-md")}
                    >
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent><p>Forget Image</p></TooltipContent>
                </Tooltip>
              </TooltipProvider>
           </div>
        </div>
  ...
  // For other types (link, note, voice, movie) or image type without imageUrl
  const hasDisplayableImage = item.imageUrl && (item.type === 'link' || item.type === 'note' || item.type === 'voice' || item.type === 'movie');
    ...
    >
      <div className="flex flex-col flex-grow group/link">
        <div className="flex flex-col flex-grow">
          ...
          <CardHeader className={cn("pb-2", !hasDisplayableImage && item.type !== 'image' ? "pt-4" : "pt-3")}>
            <div className="flex items-center gap-3 mb-2">
              {/* Icon only shown if no displayable image AND item.type is not 'image' (already handled) */}
              {!hasDisplayableImage && item.type !== 'image' && (
          ...
          <CardContent className={cn("flex-grow space-y-3", hasDisplayableImage ? "pt-0 pb-4" : "pt-2 pb-4")}>
            {item.description && (
            ...
            {/* Domain, ContentType, and Tags are intentionally removed for the experimental card */}
          </CardContent>
        </div>
      </div>
