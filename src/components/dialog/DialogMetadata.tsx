    ...
    <>
      <div className="space-y-4 pt-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {editableTags.map(tag => (<Badge key={tag.id} variant="secondary" className="px-2 py-0.5 text-xs rounded-full font-medium group relative">{tag.name}<Button variant="ghost" size="icon" className="h-5 w-5 ml-1.5 p-0.5 opacity-50 group-hover:opacity-100 hover:bg-destructive/20 hover:text-destructive absolute -right-1.5 -top-1.5 rounded-full bg-background/50" onClick={() => handleRemoveTag(tag.id)} aria-label={`Remove tag ${tag.name}`}><X className="h-3 w-3" /></Button></Badge>))}
